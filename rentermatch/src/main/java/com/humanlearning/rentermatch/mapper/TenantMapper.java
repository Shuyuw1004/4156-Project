...
//            "tPreferType = #{tPreferType} AND tPreferZipCode = #{tPreferZipCode} AND tSmoking = #{tSmoking} AND tClientId != tClientId")
  @Select(
      "SELECT tid, tAge, tClientId, tConstellation, tCooking, tEarlyTimeSleep, tExpenditure, tGender, tJob, tLateTimeSleep, "
          ...
          +
          "WHERE (tCooking = 'Yes' OR tCooking = 'No') AND tEarlyTimeSleep = #{tEarlyTimeSleep} AND (tAge >= #{tAge} - 5 AND tAge <= #{tAge} + 5) AND "
          +
          "(tExpenditure >= #{tExpenditure} - 300 AND tExpenditure <= #{tExpenditure} + 300) AND tGender = #{tGender} AND tJob = #{tJob} AND tLateTimeSleep = #{tLateTimeSleep} AND "
          +
          "(tNumOfRoomates >= #{tNumOfRoomates} - 1 AND tNumOfRoomates <= #{tNumOfRoomates} + 1 )AND (tPet = 'Yes' OR tPet = 'No') AND tPreferLocation = #{tPreferLocation} AND tPreferType = #{tPreferType} AND "
          +
