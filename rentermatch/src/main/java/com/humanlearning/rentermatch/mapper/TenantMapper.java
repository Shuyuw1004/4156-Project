...
public interface TenantMapper {
    ...
    int saveTenant(@Param("tid") String tid, @Param("tAge") Integer tAge,
    ...
    @Select("SELECT tid, tAge, tClientId, tConstellation, tCooking, tEarlyTimeSleep, tExpenditure, tGender, tJob, tLateTimeSleep, tMatches, tNumOfRoomates, tPet, tPhone, tPreferLocation, tPreferType, tPreferZipCode, tSmoking FROM coms4156.tenant WHERE tid=#{tid}")
    Tenant selectTenantBytId(@Param("tid") Integer tid);

    @Select("SELECT tid, tAge, tClientId, tConstellation, tCooking, tEarlyTimeSleep, tExpenditure, tGender, tJob, tLateTimeSleep, tMatches, tNumOfRoomates, tPet, tPhone, tPreferLocation, tPreferType, tPreferZipCode, tSmoking FROM coms4156.tenant WHERE tClientId=#{tClientId}")
    Tenant selectTenantBytClientId(@Param("tClientId") Integer tClientId);
}
